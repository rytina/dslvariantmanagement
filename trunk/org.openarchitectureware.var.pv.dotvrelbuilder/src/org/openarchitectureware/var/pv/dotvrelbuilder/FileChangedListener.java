...
public class FileChangedListener implements IResourceChangeListener {
  ...
  private void parse(IFile file) throws XmlPullParserException, IOException, CoreException {
	  ...
	  String completeFileDependency = Config.getCompleteFileDependency(file);
	  if ( completeFileDependency != null ) {
		  RelCache.getInstance().add(completeFileDependency,new ElementMapEntry(file,1));		  
	  }
	  ...
	  while ( br.ready() ) {
		  ...
		  List<String> featuresInThatLine = Config.findFeatureDependenciesInLine(file, line);
